...
export interface ChapterImages { id: string; title: string; total: number; page: number; limit: number; hasMore: boolean; images: string[]; }
...
export async function getHomepage(tag?: string) {
  const qs = tag ? `?tag=${encodeURIComponent(tag)}` : "";
  return request<{ books: SearchItem[]; tag?: string }>(`/homepage${qs}`);
}
